...
def numba_syth_resolution(coefficients,l_new,sampl,min_sampl,last_frequency):
        ...
        return l_new
def synth_resolution_degradation(wave, synth,res_map,res_b,l_new_premade=None,kernel_=None,synth_res=300000.0):
        """
...
name=large_data_GALAH_official[200]['sobject_id']
...
for value,x in enumerate(large_data_GALAH_official):
    ...
    old_abundances=vstack(old_abundances)
    filename = 'machine_learning_long/'+str(name)+'_old_moves_machine_learning_prior.h5'
    filename2= 'machine_learning_long/'+str(name)+'_old_moves_machine_learning_no_prior.h5'
    ...
    pos=starter_walkers_maker(nwalkers)
    ndim=np.shape(pos)[1]
    backend = emcee.backends.HDFBackend(filename)
    backend.reset(nwalkers, ndim)
    
    with Pool(processes=nwalkers) as pool:
        sampler = emcee.EnsembleSampler(nwalkers, ndim, log_posterior,pool=pool,backend=backend,args=[prior])
