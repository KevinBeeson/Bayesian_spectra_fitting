...
class spectrum_all:
    ...
    def __init__(self,name,interpolation):
        ...
        for x in bands:
            ...
            rsetattr(self,x+'.linelist',ValdFile('Green_temp.lin'))
...
def line_splitter(start,end):
    ...
    line_list_out=line_list_long[int(start*len(line_list_long)-start*len(line_list_long)%4):int(end*len(line_list_long)-end*len(line_list_long)%4)]
    starting_segment=line_list_long[:int(start*len(line_list_long)-start*len(line_list_long)%4)]
    ending_sgement=line_list_over_lap[int(end*len(line_list_long)-end*len(line_list_long)%4):]
    ...
    line_list_with=[]
    #sees if its a duplicate
    starting_segment_with=[]
    for value,x in enumerate(starting_segment[::4]):
        name=x.split(',')[0]
        if name in short_line_list_values[:,0]:
            ...
            if wave_to_find in   wavelength:
                starting_segment_with.append(line_list_long[value*4:value*4+4])
    ending_segment_with=[]
    for value,x in enumerate(ending_sgement[::4]):
        name=x.split(',')[0]
        if name in short_line_list_values[:,0]:
            points=np.where(short_line_list_values[:,0]==name)
            rest=[y for y in short_line_list_values[points]]
            ...
            wave_to_find=[round(y,4) for y in wave_to_find]
            if wave_to_find in   wavelength:
                ending_segment_with.append(line_list_long[value*4:value*4+4])    
    #writes it all
    ...
    for x in header:
        big_string+=x
    for x in starting_segment:

        for y in x:
            big_string+=y

    for x in line_list_out:

        for y in x:
            big_string+=y
    for x in ending_sgement:

        for y in x:
            big_string+=y
    big_string+=refrences
    with open('Green_temp.lin','w') as output:
        # for x in header:
        #     output.write(str(x))
        output.write(big_string)

name=131217003901022
filename = "131217003901022"
...
start=5
n_2=3
line_splitter(0.125,0.25)
while found:
    start=2**n_2 +1
    split=np.linspace(0,1,num=start)
    
    for value in range(len(split[:-1])):
        line_splitter(split[value],split[value+1])
        spectras=spectrum_all(name,1)
        print('synthesizing from ', split[value],' until ', split[value+1])
        spectras.synthesize({})
        if min(spectras.Green.synth[0][3:-3])>0.99:
            print('here')
        else:
            print('still breaking')
            found=False
            break
    n_2+=1
