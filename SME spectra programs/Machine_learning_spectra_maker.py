...
# pos=np.column_stack((teff_random,vsini_random,logg_random,v_rad_random,monh_random))
    ...
    # print("Spectra",synthetic_spectras)
    return probability + log_prior(shift)
def starting_test(solar_values):
    ...
    if 'teff' in shift:
         if min(abs(shift['teff']-old_abundances['teff'])-abs(old_abundances['teff']-old_abundances['teff_gaia']),0)/old_abundances['e_teff']>limit:
    ...
    if 'logg' in shift:
        if shift['logg']<1.0 or shift['logg']>5.0:
            ...
            return False  
        if min(abs(shift['logg']-old_abundances['logg'])-abs(old_abundances['logg']-old_abundances['logg_gaia']),0)/old_abundances['e_logg']>limit:
            print("logg is too small or big")
            return False  
...
class individual_spectrum:
    def __init__(self,name,interpolation,x,count):
            ...
            # if not exists(name[0:6]+'/spectra/com/'+name+str(x)+'.fits'):
            ...
            self.vsini=float(old_abundances['vbroad'])
            self.vrad=float(old_abundances['rv_galah'])
            self.vmac=6.0
    ...
    def synthesize(self,shift,multi=False,colours=bands,give_back=False):
        if not (multi or give_back):
            for x in colours:       
                ...
                synthesized_spectra=synth_resolution_degradation(rgetattr(self,x+'.wave'),spectrum,rgetattr(self,x+'.hermes')[7].data,rgetattr(self,x+'.hermes')[7].header['b'])[:,1]
                if 'vrad' in shift:
                    dopler_shifted_spectra=dopler(rgetattr(self,x+'.wave'),synthesized_spectra,shift['vrad'])
                else:
                    dopler_shifted_spectra=dopler(rgetattr(self,x+'.wave'),synthesized_spectra,rgetattr(self,x+'.vrad'))
...
reader= emcee.backends.HDFBackend('160106001601282_machine_learning.h5')
fitted_data=reader.get_chain(flat=True)
fitted_data=np.mean(fitted_data,axis=0)

shift=shift_maker(fitted_data)
...
# for x,colour in enumerate(bands):
#     plt.figure()
#     wave=rgetattr(spectras,colour+'.wave')
#     plt.plot(wave,synthesized_spectra[x],c='black',label='Machine learning spectra')
#     plt.plot(wave,,c='r',label='pysme spectra')
#     plt.legend(loc='best')
#     plt.title(colour)
