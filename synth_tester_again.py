...
def synth_resolution_degradation_svens(l, res_map, res_b, synth, synth_res=300000.0, reuse_initial_res_wave_grid=True, initial_l=None):
    """
...
def galah_kern(fwhm, b):
        ...
        return g / np.sum(g)
def dopler(original_wavelength,v_rad,synthetic_wavelength,grad=None):
        """
            ...
            for value,x in enumerate(colours):
                ...
                masks_temp=(
                    (
                        ...
                        ([False for x in range(len(wave_opt))])
                    ) |
        ...
        if not give_back:
            if not multi:
                for x in colours:       
                    ...
                    if 'vrad_'+x in shift:
                        dopler_shifted_synth_wave=dopler(rgetattr(self,x+'.wave'),shift['vrad_'+x],synthetic_wavelength=rgetattr(self,x+'.wave_synth'))
                    else:
                        dopler_shifted_synth_wave=dopler(rgetattr(self,x+'.wave'),rgetattr(self,x+'.vrad'),synthetic_wavelength=rgetattr(self,x+'.wave_synth'))
...
spectras=spectrum_all(name,cluster=prior)
t1=time.time()
spectras.synthesize()
t0=time.time()
t2=time.time()
synth=spectras.synthesize(give_back=(True))
t3=time.time()
...
# spectras.create_masks(clean=True)
# print(filename)
# old_abundances=spectras.old_abundances
# if prior:
...
# pos_short=starter_walkers_maker(nwalkers,old_abundances,parameters_no_vrad,cluster=prior)
...
# sampler = emcee.EnsembleSampler(nwalkers, ndim, log_posterior,backend=backend,args=[parameters_no_vrad,prior,True,parameters])
...
# pos_long=starter_walkers_maker(nwalkers,old_abundances,parameters_no_vrad,cluster=prior)
...
# # sampler = emcee.EnsembleSampler(nwalkers, ndim, log_posterior,backend=backend,args=[parameters_no_vrad,prior,normalized_limit_array,parameters],a=10)
...
# log_posterior(pos_long[0],parameters_no_vrad,prior,normalized_limit_array,parameters)
# autocorr=[]
# index=0
...
# file_directory = cluster_name+'_reduction/plots/prior'
# Path(file_directory).mkdir(parents=True, exist_ok=True)
