...
def runGetattr(obj, attr):
    ...
    return functools.reduce(_getattr, [obj] + attr.split('.'))
def starting_test(solar_values,old_abundances,parameters=['teff','logg','monh','fe_h','alpha','vrad_Blue','vrad_Green','vrad_Red','vrad_IR','vsini','vmac','vmic']):
        """
        ...
        radial_velocities_r=[old_abundances['rv_com'] for x in vrad_r]
    ...
    def hermes_checker(self,limit=20,colours=None):
        ...
        for x in colours:
            ...
            if max(spectra)>limit:
                ...
                return True
        return False
    def create_masks(self,colours=None,synthetic_spectra_insert=None,uncs_insert=None,normalized_observed_spectra_insert=None,shift=None,clean=False):
        if colours==None:
            colours=self.bands
...
def starter_walkers_maker(nwalkers,old_abundances,parameters=['teff','logg','monh','fe_h','alpha','vrad_Blue','vrad_Green','vrad_Red','vrad_IR','vsini','vmac','vmic'],cluster=False):
    ...
    elements=['Li','C','N','O','Na','Mg','Al','Si','K','Ca','Sc','Ti','V','Cr','Mn','Co','Ni','Cu','Zn','Rb','Sr','Y','Zr','Mo','Ru','Ba','La','Ce','Nd','Sm','Eu']
    old_abundances_labels={'alpha':'alpha_fe_r','fe_h':'fe_h_r','vsini':'vbroad_r','vmic':'vmic_r','vmac':'vmic_r','vrad_Blue':'rv',
...
name=cross_data['sobject_id'][0]
filename = 'NGC_2682_reduction_dr61/'+str(name)
# if os.path.exists(filename+'_radial_velocities.h5') and os.path.exists(filename+'_all_elements.h5'):
#       print('already done '+ str(name))
#       continue

spectras=spectrum_all(name,cluster=False,bands=['Blue'])
synth_out=spectras.synthesize(give_back=True)
spectras_janez=gtools.spectrum(str(name)+'1',kind='flux')
spectras_janez.equalize_resolution()
synth_in=np.vstack((spectras.Blue.wave,synth_out[0]))
synth_janes=spectras_janez.synth_resolution_degradation(synth_in.T)
# print(filename)
...
# pos_short=pos[:len(params_short)*2,:len(params_short)]
...
# first_try=True
# log_posterior(pos_short[0],params_short,False,True,parameters)
# step_iteration=20
...
# sampler = emcee.EnsembleSampler(nwalkers, ndim, log_posterior,backend=backend,args=[params_short,prior,True,parameters])
...
#             continue
# shift_temp=shift_maker(np.mean(sampler.get_chain(flat=True,discard=sampler.iteration//2 ),axis=0),params_short,False,parameters)   
...
# pos_long=np.hstack((pos[:len(parameters_no_vrad)*2,:5],pos[:len(parameters_no_vrad)*2,9:]))
...
# sampler = emcee.EnsembleSampler(nwalkers, ndim, log_posterior,backend=backend,args=[parameters_no_vrad,prior,normalized_limit_array,parameters])
...
# for sample in sampler.sample(pos_long,iterations=60, progress=True):
#     if sampler.iteration % step_iteration:
#             continue
...
# info_line_3='rv Blue='+ str(np.round(shift_temp['vrad_Blue'],decimals=2))+'km/s, ' +\
#         'rv Green='+ str(np.round(shift_temp['vrad_Green'],decimals=2))+'km/s, ' +\
#         'rv Red='+ str(np.round(shift_temp['vrad_Red'],decimals=2))+'km/s, ' +\
#         'rv IR='+ str(np.round(shift_temp['vrad_IR'],decimals=2))+'km/s'
# fig=plot_spectrum(
#     wave,
#     [
#         obs,
...
# file_directory = 'NGC_2682_reduction_dr61/plots/'
...
#     shift_temp=shift_maker(np.mean(sampler_rad.get_chain(flat=True,discard=20),axis=0),params_short,False,parameters)   
