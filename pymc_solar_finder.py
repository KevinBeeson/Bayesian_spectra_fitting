...
for lab in labels:
    ...
    for value,rad in enumerate(np.linspace(-10, 10,num=how_many_starting)):
        
        log_temp[value]=spectras.log_fit(shift={lab:rad},self_normal=(False))
        grad_temp[value]=spectras.gradient(shift={lab:rad},labels=[lab])
    plt.plot(np.linspace(-10, 10,num=how_many_starting),log_temp,label=lab)
    plt.plot(np.linspace(-10, 10,num=how_many_starting),grad_temp,label=lab+'grad')

    starting_radial.append(np.linspace(-10, 10,num=how_many_starting)[np.where(log_temp==max(log_temp))][0])
plt.legend(loc='best')
plt.figure()
log_temp=np.ones(how_many_starting)
grad_temp=np.ones(how_many_starting)
...
# with pm.Model():
    ...
    # # a=pm.find_MAP()
    # idata_mh = pm.sample(2000, tune=2000,return_inferencedata=True,cores=4)
