...
class spectrum_all:
    ...
    def limit_array(self,colours=None,limit=1.1,observed_spectra=None,pixels=3):
        if colours==None:
            colours=self.bands
        if not np.array_equal(observed_spectra,None):
            ...
            for value,spec in enumerate(observed_spectra):
                ...
                rows=len(limit_array_temp)
                limit_array_temp_temp=np.ones(len(limit_array_temp))
                for i in range(rows):
                    if limit_array_temp[i]==0:
                        for move in range(-pixels,pixels+1):
                            if move+i>0 and move+i<len(limit_array_temp):
                                limit_array_temp_temp[move+i]=0
                returning_limit[value]=limit_array_temp_temp
        ...
        for x in colours:
            if rgetattr(self, x+'.hermes')!=None:
                ...
                rows=len(limit_array)
                limit_array_temp_temp=np.ones(len(limit_array))
                for i in range(rows):
                    if limit_array[i]==0:
                        for move in range(-pixels,pixels+1):
                            if move+i>0 and move+i<len(limit_array):
                                limit_array_temp_temp[move+i]=0
                rsetattr(self,x+'.limit',limit_array_temp_temp)
...
for star in photometric_data:
    ...
    filename = cluster_name+'_reduction/no_prior_reduced_elements_2_'+str(name)
...
#        pos_long=sampler.get_chain()[-1]        
        ...
        Path(file_directory).mkdir(parents=True, exist_ok=True)
        file_directory+='prior_reduced_elements_2'
